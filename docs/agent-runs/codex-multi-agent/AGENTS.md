...
- [MA-17] MA-17 should hard-fail when benchmark prerequisites are absent; no optimization patch should land without before/after evidence.
- [MA-17] Structural script/workflow readiness is not a substitute for authoritative runtime measurement in optimization acceptance.
- [MA-18] Structural readiness (workflow/scripts/schema/docs) and authoritative runtime proof must remain explicitly separated in acceptance logic.
- [MA-18] Final acceptance quality improves when unsupported optimistic findings are explicitly discarded instead of softened into partial PASS language.
<!-- END AUTO-LEARNINGS -->
